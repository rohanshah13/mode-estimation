...
from util import write_data, write_data_seed
...
def ppr_evaluate(party_distribution, population_size, alpha=0.05, iterations=100, log=True):
	'''
	...
	for i in range(iterations):
		...
		oracle = communitySampleOracle(i, population_size, party_distribution)
...
if __name__ == '__main__':
	...
	[num_samples, num_success, time_taken] = ppr_evaluate(party_distribution, args.population_size, args.delta, args.iterations)    

	std = np.std(num_samples)
	std_time = np.std(time_taken)

	avg_samples = sum(num_samples) / args.iterations
	avg_success = sum(num_success) / args.iterations
	avg_time = sum(time_taken) / args.iterations

	se = std/np.sqrt(args.iterations)
	se_time = std_time/np.sqrt(args.iterations)

	failure_probability = str(1 - avg_success)
	
	print("Failure probability is "+str(1-avg_success))
	print("Average number of samples are "+str(avg_samples))
	...
	write_data_seed('PPR-Bernoulli-1v1',num_samples, args.delta, num_success, args.outfile)